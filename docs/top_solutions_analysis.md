...
## 4. 我々の現在地と、正攻法で 0.115 台を詰めるには

現在：**正攻法 LB 0.12246 / CV 0.1079**（リーク実演は 0.00463）。
前処理・モデルはトップ正攻法とほぼ同等。CV(0.108)と LB(0.122)の差は、
train/test を結合して前処理した分 **CV が楽観的に出ている**ため。

ここから 0.115 台へ詰めるなら（労力順）：

1. **CV からリークを排した厳密な検証基盤**：前処理を sklearn Pipeline 化し、
   fold 内でのみ fit する。CV が信頼できれば改善判断が正確になる（今は CV が甘く、
   改善の良し悪しを LB 提出でしか確かめられない）。**最優先・最重要**。
2. **アンサンブル構造の調整**：Serigne の `0.70/0.15/0.15` のような手調整ブレンドや、
   2段スタッキングの試行。
3. **残差分析 → 特徴量追加**：外しているのがどんな物件かを見て、的を絞った特徴量を作る。
4. **モデルのハイパラ最適化**（Optuna など）。ただし効果は逓減。

逆に言えば、**純ML・正攻法での実用上の底は 0.115 前後**で、そこから先（0.10 以下）は
リーク領域。趣味としては 0.12 前後で十分に「トップ正攻法の入り口」に立てている。
